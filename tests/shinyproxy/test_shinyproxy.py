...
class HelmChart(object):
    ...
    def run_helm_template(self, files):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            helm_command = self.render_helm_template_command(
                 temp_dir=tmpdirname
            )
            ...
            for base_file in os.listdir(template_dir):
                ...
                if os.path.isfile(file) and base_file in files or base_file == 'configmap.yaml':
...
def test_values_social():
    ...
    assert secrets_json['proxy']['social']['facebook']['app-secret'] == 'yourfacebookappsecret'
...
def test_values_specs_service_account():
    ...
    k8s_pod_patches = chart.application_data['proxy']['specs'][0]['kubernetes-pod-patches']
    ...
    assert chart.application_data['proxy']['specs'][0]['service-account'] == k8s_pod_patches_data[0]['value']
    # pprint(chart.application_data)
    # pprint(k8s_pod_patches_data)